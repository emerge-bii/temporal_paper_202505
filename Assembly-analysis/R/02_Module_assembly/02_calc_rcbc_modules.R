...
if (length(args)==1) { # args will always be >= 1 b/c of the --show-progress flag in mpiexec
  ...
  writeLines(paste0("Calculating RCBC for ", args[1]))
...
if(speedup == TRUE) {
  betaNTI <- read_csv(file = paste0(outputs.fp, "/", habitat_name, "_", module_name, "_", "weighted_bNTI.csv")) %>%
...
# if the maximum abundances are < 1, this will mess up calculating the differential
# abundances in Raup-Crick because you will have a negative number of species draws.
# instead, we will multiply spXsite by 100 which will not change the relative abundances.
# of each organism, but will allow us to draw species in appropriate proportions for null
# distribution
if(rel_abund == TRUE && min(apply(spXsite, MARGIN = 1, max)) < 1){ # less than 2 rather than less than 1 b/c we've already added 1 to everything
  spXsite <- spXsite*100
}
...
create_spec_comb <- function(nsites = nrow(spXsite), nreps = beta.reps, speedup = FALSE) {
...
create_null <- function(site_comb, spXsite = spXsite) {
  ...
  # In first sample, choose a random subset of species to form the community: 
  # x = choose from any of the gamma # of species in the region, 
  # size = choose a vector of species with a size equal to the number of species in original community; 
  # Weigh the probability of choosing a particular species by its regional occurrence frequency ("occur"); 
  # Then assign this random subset of regional species a presence value of "1"
  com1[sample(1:gamma, sum(spXsite.inc[null.one,]), replace=FALSE, prob=occur)]<-1
  
  # using the pool of randomly chosen species in com1, 
  # x = the randomly selected species in com 1 (which are equal to the number of species in the original community, 
  # but don't necessarily have the same identity)
  # size = the difference in total number of individuals in the real community and total number of observations (pres/abs)
  # (which is equivalent to sum(com1), and also sum(spXsite.inc[null.one,]))
  # in the community. Essentially, the abundances will be randomly re-distributed across the new null community
  # IMPORTANT: the number of individuals observed (sum(spXsite[null.one,])) must
  # be greater than the number of observations (sum(com1)) 
  # The chance of choosing a species is propotionate to those species' abundances in the 
  # real regional community,however species may be chosen with replacement, 
  # so it is possible to pick the same species twice or more
  # Bug alert! if com1 only has one species >0, then sample() fails
  if(length(which(com1>0)) == 1) { # This loop handles that
    # This is a workaround to allow sampling from a community with only one species > 0.
    # we artificially add a "0" species to the options, and then assign the probability
    # of choosing that species to 0. 
    samp_options <- c(which(com1>0),0)
    prob_samp_options <- c(abundance[which(com1>0)],0)
    
    com1.samp.sp <- sample(samp_options, (sum(spXsite[null.one,])-sum(com1)), 
                           replace=TRUE, prob=prob_samp_options); # random draws of species from the com1 community; chosen propotionate to their regional abundances. The total size of the community is bounded by the size of the real community membership
  ...
  #sum(com1) - sum(spXsite[null.one,]); # This should be zero if everything worked properly (or between -1 and 1 if your counts are fractional)
  rm('com1.samp.sp','com1.sp.counts');			
  #writeLines("Community 1 calculated")
  
  # Again for com2
  com2[sample(1:gamma, sum(spXsite.inc[null.two,]), replace=FALSE, prob=occur)]<-1
  if(length(which(com2>0)) == 1) {
    # This is a workaround to allow sampling from a community with only one species > 0.
    # we artificially add a "0" species to the options, and then assign the probability
    # of choosing that species to 0. 
    samp_options <- c(which(com2>0),0)
    prob_samp_options <- c(abundance[which(com2>0)],0)
    
    com2.samp.sp <- sample(samp_options, (sum(spXsite[null.one,])-sum(com2)), 
                           replace=TRUE, prob=prob_samp_options); # random draws of species from the com1 community; chosen propotionate to their regional abundances. The total size of the community is bounded by the size of the real community membership
    
  } else {
    com2.samp.sp <- sample(which(com2>0), (sum(spXsite[null.two,])-sum(com2)), 
                           replace=TRUE, prob=abundance[which(com2>0)]);
  }
...
if(!skip_null) {
  ...
  tmp_null <- here(outputs.fp, paste0("tmp_", habitat_name, "_", module_name))
  print(tmp_null)
...
# the final combination object might be too big to hold in memory. 
# Writing the outputs to temporary files and then reading them in solves that problem
tmp_comp <- here(outputs.fp, paste0("tmp_2_", habitat_name, "_",  module_name))
...
# Data
# RCBC matrix
saveRDS(results.wf, paste0(outputs.fp, "/", habitat_name, "_", module_name, "_", "rcbc_matrix.RDS"))
write.csv(results.wf,
          paste0(outputs.fp, "/", habitat_name, "_", module_name, "_",  "rcbc_matrix.csv"),
          quote=FALSE)

# Long formats outputs from foreach loops
write.csv(results.lf,
          paste0(outputs.fp, "/", habitat_name, "_", module_name, "_",  "rcbc_long_form.csv"),
